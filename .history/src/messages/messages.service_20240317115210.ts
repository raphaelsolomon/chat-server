...
export class MessagesService {
  ...
  async createMessage(
    ...
    if (!selectedChat) {
      selectedChat = this.chat.create({
        name: 'p2p',
        participants: [createMessageDto.sender_uuid, createMessageDto.receiver_uuid],
        admin: createMessageDto.s
      });
    }
    ...
    client.in(selectedChat.uuid).emit('messageReceived', message);
    // this.messages.push({});
    return message;
  }
