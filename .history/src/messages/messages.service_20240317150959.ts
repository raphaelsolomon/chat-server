...
export class MessagesService {
  constructor(
    @InjectModel('Chat') private readonly chatModel: Model<I>,
    @Inject('CHAT_MESSAGE_MODEL') private chatMessageModel: Model<Chat>,
  ) {}
  ...
  async createMessage(
    ...
    const getChat = this.chat.aggregate([
      {
        $match: {
          isGroupChat: false,
          $and: [
    ...
    const chats = await getChat.toArray();

    if (chats.length) {
      selectedChatMessage = chats[0];
    } else {
